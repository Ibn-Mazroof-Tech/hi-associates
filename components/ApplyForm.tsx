...
if (emailTrimmed && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailTrimmed)) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      // the real Apps Script webhook URL stays a server-only secret and
      ...
      // The Ticket ID is generated server-side (Apps Script), not here —
      // this keeps it sequential and collision-free even if two people
      // submit at the same moment.
      if (data.result === "success" && data.ticketId) {
        setLastTicketId(data.ticketId);
        router.push(
          `/application-submitted?ticket=${encodeURIComponent(data.ticketId)}&service=${encodeURIComponent(
